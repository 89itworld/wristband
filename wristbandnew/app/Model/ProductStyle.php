...
class ProductStyle extends AppModel
{
    ...
    var $validate = array(
        ...
        'name' => array(
            'notempty' => array(
                'rule' => 'notempty',
                'message' => 'Please enter product style name.',
                 "last" => true
            ),
            'product_size_unique' => array(
                'rule' => array('product_size_unique','product_category_id'),
                'message' => 'This product style name has been already taken.',
            )
        ),
    ...
    function product_size_unique($data = array(), $compare_field = null)
    {
       $cat_id =$this->data[$this->name][$compare_field];
        try {
            $data = $this->find('first', array('conditions' => array('ProductStyle.name' => $data["name"],'ProductStyle.product_category_id' => $cat_id)));
            if (!empty ($data)) {
                return false;
            } else {
                return true;
